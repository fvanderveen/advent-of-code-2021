...
fn puzzle1(input: &String) {
    let mut grid: Grid<usize> = input.parse().unwrap();
...
fn run_flash_cycle(grid: &mut Grid<usize>) -> usize {
...
fn get_flashes_after(grid: &mut Grid<usize>, num_cycles: usize) -> usize {
    let mut result = 0;
...
fn find_step_all_flash(grid: &mut Grid<usize>) -> usize {
    let expected_amount = grid.bounds.width * grid.bounds.height;
    let mut step = 1;
    loop {
...
mod tests {
    ...
    fn test_find_step_all_flash()
    {
        ...
        assert_eq!(grid, vec![vec![0; 10]; 10].try_into().unwrap())
    }
}